...
@section('content')
    <div class="flex gap-20 justify-center md:my-7 p-14 md:mx-20 bg-[]">
        <h1 class="text-2xl md:text-6xl font-bold font-heading text-[#424242] [text-shadow:_2px_2px_4px_rgb(0_0_0_/_15%)]">あなたのお気に入りレシピ<i class="fa-solid fa-star"></i></h1>
    </div>
                ...
                <a href="{{ route('recipes.show', $favorite->recipe->id) }}" class="flex flex-col h-full relative">
                    @if($favorite->recipe->image)
                        <div class="relative">
                            ...
                            <img src="{{ asset('storage/' . $favorite->recipe->image) }}" alt="{{ $favorite->recipe->title }}" class="w-full h-32 md:h-48 object-cover rounded-t-lg">
                        </div>
                    @else
                        <div class="relative">
                            <div class="absolute top-2 left-0 right-0 flex justify-center space-x-2 z-10">
                    ...
                    <div class="p-4 flex-1 flex flex-col">
                        <div class="flex flex-col md:flex-row md:items-center">
                            <p class="text-sm text-gray-500">{{ $favorite->recipe->updated_at->format('Y/m/d') }}</p>
                            <p class="text-sm text-gray-500 mb-2 md:mb-0 md:ml-4">調理時間：{{ $favorite->recipe->cooking_time }}分</p>
                        </div>
                        <h2 class="text-lg md:text-xl text-[#622d18] font-bold mb-2 line-clamp-2">{{ $favorite->recipe->title }}</h2>
